...
contract HyperlaneDSS is IHyperlaneDSS, OwnableUpgradeable {
    ...
    bytes32 internal constant STORAGE_SLOT = 0x97a737615475bcee79cdac5f4bd6f79c0343e9fd71e6afea38865e795c47e800;
     ...
     */
    function initialize(address _owner, address _core, uint256 _minWeight) external initializer {
        _transferOwnership(_owner);
        _self().init(_core, _minWeight);
    }
     ...
     */
    function enrollIntoChallengers(IRemoteChallenger[] memory challengers)
        external
        onlyRegisteredOperator(msg.sender)
    {
     ...
     */
    function startUnenrollment(IRemoteChallenger[] memory challengers) external onlyRegisteredOperator(msg.sender) {
     ...
     * @param challengers The list of challengers to unenroll from
     */
    function completeUnenrollment(address[] memory challengers) external onlyRegisteredOperator(msg.sender) {
     ...
     */
    function jailOperator(address operator) external virtual onlyEnrolledChallenger(operator) {
     ...
     */
    function updateOperatorSigningKey(address newSigningKey) external onlyRegisteredOperator(msg.sender) {
    ...
    function updateOperators(address[] memory operators) public {
        ...
        for (uint256 i = 0; i < operators.length; i++) {
            if (!self.isOperatorRegistered(operators[i])) revert OperatorNotRegistered();
     ...
     */
    function updateQuorumConfig(HyperlaneDSSLib.Quorum memory quorum, address[] memory operators) external onlyOwner {
     ...
     */
    function updateMinimumWeight(uint256 newMinimumWeight, address[] memory operators) external onlyOwner {
     ...
     */
    function isValidSignature(bytes32 dataHash, bytes memory signatureData) external view returns (bytes4) {
        ...
        _self().checkSignatures(dataHash, operators, signatures, referenceBlock);
     ...
     */
    function getLastestOperatorSigningKey(address operator) external view returns (address) {
     ...
     */
    function getChallengerEnrollment(address operator, IRemoteChallenger _challenger)
        external
        view
     ...
     * @param operator The address of the operator
     */
    function getOperatorChallengers(address operator) public view returns (address[] memory) {
    ...
    function getOperatorRestakedVaults(address operator) external view virtual returns (address[] memory) {
        return _self().getOperatorRestakedVaults(operator);
    }

    function isOperatorJailed(address operator) public returns (bool) {
        _self().isJailed(operator);
    }
    ...
    modifier onlyEnrolledChallenger(address operator) {
        if (_self().getEnrollmentStatus(operator, msg.sender).status == EnrollmentStatus.UNENROLLED) {
            revert OperatorNotEnrolledWithChallenger(IRemoteChallenger(msg.sender));
        }
        _;
    }
    ...
    modifier onlyRegisteredOperator(address operator) {
        if (!_self().isOperatorRegistered(operator)) revert OperatorNotRegistered();
        _;
    }
}
